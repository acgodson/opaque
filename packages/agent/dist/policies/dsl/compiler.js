...
export class PolicyCompiler {
    ...
    generateSummary(policy) {
        ...
        parts.push(`Transfer up to ${amount} ${currency} every ${this.periodToHuman(period)}`);
        // Conditions
        if (policy.conditions) {
            const { timeWindow, signals, recipients, cooldown } = policy.conditions;
            // Time window
            if (timeWindow) {
                ...
                const timeStr = this.timeRangeToHuman(timeWindow.startHour, timeWindow.endHour);
            ...
            // Security monitoring
            if (signals?.security) {
                parts.push(`pause if more than ${signals.security.maxAlertCount} security alerts`);
            }
            // Recipients
            if (recipients?.allowed && recipients.allowed.length > 0) {
                parts.push(`only to ${recipients.allowed.length} whitelisted address${recipients.allowed.length > 1 ? "es" : ""}`);
            }
            else if (recipients?.blocked && recipients.blocked.length > 0) {
                parts.push(`excluding ${recipients.blocked.length} blocked address${recipients.blocked.length > 1 ? "es" : ""}`);
            }
            // Cooldown
            if (cooldown) {
                parts.push(`with ${this.secondsToHuman(cooldown.seconds)} minimum between transactions`);
            }
        }
     ...
     */
    timeRangeToHuman(startHour, endHour) {
